...
const VAD_SPEECH_THRESHOLD_MS = 200;  // 连续语音超过此值判为"说话中"
const VAD_SILENCE_THRESHOLD_MS = 800; // 连续静音超过此值判为"语音结束"
const FRAME_ENERGY_THRESHOLD = 0.003;  // 帧能量阈值（低于此判静音）
...
class VAD {
  constructor() {
    ...
    this.speechDuration = 0;      // 当前连续语音时长(ms)
    this.silenceDuration = 0;     // 当前连续静音时长(ms)
...
async function processAudioPipeline(session) {
  ...
  // 最小音频长度检查（至少 100ms 的 16kHz 16bit PCM）
  ...
  try {
    ...
    // OpenAI TTS 返回 24kHz PCM，需要降采样到 16kHz
    ...
    // 分块发送（每块 ~100ms）
    const chunkSize = Math.floor(SAMPLE_RATE * 0.1); // 100ms @ 16kHz
...
// 人设头像 API（返回 base64 头像）
app.get("/api/personas/:id/image", (req, res) => {
  const personaId = req.params.id;
  // 返回一个占位头像 - 实际使用时会嵌入 base64 的头像
  res.sendFile(path.join(__dirname, "web", "placeholder-avatar.png"), (err) => {
    if (err) {
      // 返回 SVG 占位图
